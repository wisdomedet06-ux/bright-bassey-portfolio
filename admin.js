...
function itemEditor(title,fields,onSave,onDelete,meta={}){const key=(f)=>String(f.value??'').replace(/"/g,'&quot;');return `<div class="item" data-table="${esc(meta.table||'')}" data-id="${esc(meta.id||'')}"><div class="item-title"><b>${esc(title)}</b><div><button class="secondary save-item">Save</button><button class="danger delete-item">Delete</button></div></div><div class="form-grid">${fields.map(f=>`<label class="field ${f.wide?'wide':''}"><span>${esc(f.label)}</span>${f.textarea?`<textarea name="${esc(f.key)}">${esc(f.value??'')}</textarea>`:`<input name="${esc(f.key)}" value="${key(f)}">`}</label>`).join('')}</div></div>`}
function renderExperience(){const el=$('#experienceList');el.innerHTML=cache.experience.map(x=>itemEditor(x.role,[{label:'Role',key:'role',value:x.role},{label:'Company',key:'company',value:x.company},{label:'Period',key:'period',value:x.period},{label:'Bullets (one per line)',key:'bullets',value:(x.bullets||[]).join('\n'),textarea:true,wide:true}],async obj=>{obj.bullets=obj.bullets.split('\n').map(s=>s.trim()).filter(Boolean);const {error}=await supabase.from('experience').upsert({id:x.id,...obj,sort_order:x.sort_order});if(error)throw error;toast('Experience saved');loadAll()},async()=>{await supabase.from('experience').delete().eq('id',x.id);toast('Experience deleted');loadAll()},{table:'experience',id:x.id})).join('')}
function renderSkills(){const el=$('#skillsList');el.innerHTML=cache.skills.map(x=>itemEditor(x.category,[{label:'Category',key:'category',value:x.category},{label:'Tags',key:'tags',value:x.tags,wide:true},{label:'Icon',key:'icon',value:x.icon}],async obj=>{const {error}=await supabase.from('skills').upsert({id:x.id,...obj,sort_order:x.sort_order});if(error)throw error;toast('Skill saved');loadAll()},async()=>{await supabase.from('skills').delete().eq('id',x.id);toast('Skill deleted');loadAll()},{table:'skills',id:x.id})).join('')}
function renderProjects(){const el=$('#projectsList');el.innerHTML=cache.projects.map(x=>itemEditor(x.name,[{label:'Name',key:'name',value:x.name},{label:'Category',key:'category',value:x.category},{label:'URL',key:'url',value:x.url,wide:true},{label:'Description',key:'description',value:x.description,textarea:true,wide:true},{label:'Tags separated by ·',key:'tech',value:x.tech,wide:true}],async obj=>{const {error}=await supabase.from('projects').upsert({id:x.id,...obj,sort_order:x.sort_order});if(error)throw error;toast('Project saved');loadAll()},async()=>{await supabase.from('projects').delete().eq('id',x.id);toast('Project deleted');loadAll()},{table:'projects',id:x.id})).join('')}
function renderEducation(){const el=$('#educationList');el.innerHTML=cache.education.map(x=>itemEditor(x.title,[{label:'Title',key:'title',value:x.title},{label:'Body',key:'body',value:x.body,wide:true},{label:'Type',key:'type',value:x.type}],async obj=>{const {error}=await supabase.from('education').upsert({id:x.id,...obj,sort_order:x.sort_order});if(error)throw error;toast('Credential saved');loadAll()},async()=>{await supabase.from('education').delete().eq('id',x.id);toast('Credential deleted');loadAll()},{table:'education',id:x.id})).join('')}
...
$('#profileForm')
function wireDelegation(containerId){const el=$(containerId);el.addEventListener('click',async e=>{if(e.target.classList.contains('save-item')){try{const item=e.target.closest('.item');const tableName=item.dataset.table;const id=item.dataset.id;if(!tableName||!id)throw new Error('Unable to identify the record to save.');const obj={};item.querySelectorAll('input[name],textarea[name]').forEach(f=>obj[f.name]=f.value);if(tableName==='experience'){obj.bullets=obj.bullets.split('\n').map(s=>s.trim()).filter(Boolean)};const {error}=await supabase.from(tableName).upsert({id,...obj});if(error)throw error;toast('Saved');await loadAll()}catch(err){toast(`Save failed: ${err.message}`,false)}} if(e.target.classList.contains('delete-item')){try{const item=e.target.closest('.item');const tableName=item.dataset.table;const id=item.dataset.id;if(!tableName||!id)throw new Error('Unable to identify the record to delete.');const {error}=await supabase.from(tableName).delete().eq('id',id);if(error)throw error;toast('Deleted');await loadAll()}catch(err){toast(`Delete failed: ${err.message}`,false)}}})}
