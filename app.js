...
const fallback = {
  ...
  stats:[{value:'3+',label:'Years Experience'},{value:'80%',label:'Response Turnaround'},{value:'10+',label:'Tools & Platforms'},{value:'Multi-Channel',label:'Support'}],
...
function render(){
  ...
  q('#heroStats').innerHTML=data.stats.map(s=>{const value=String(s.value??'').replace(/5\+/g,'3+'); return `<div class="stat-circle"><div class="stat-num">${esc(value)}</div><div class="stat-label">${esc(s.label)}</div></div>`}).join('');
  ...
  const stats=(data.stats||[]).slice(0,4); q('#resumeStats').innerHTML=stats.map(s=>{const value=String(s.value??'').replace(/5\+/g,'3+'); return `<div><strong>${esc(value)}</strong><span>${esc(s.label)}</span></div>`}).join('');
